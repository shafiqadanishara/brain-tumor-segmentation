# =========================
# test_ensemble.py — FIXED
# Perubahan dari versi sebelumnya:
# 1. bbox dari meta dipakai saat restore_to_original
# 2. save_nifti sekarang simpan label map (D,H,W) bukan (D,H,W,3)
...
# --------------------------------------------------
# SUMMARY / COMPARISON (tidak berubah)
# --------------------------------------------------
...
def save_summary(path, flair_vol, t1_vol, t1ce_vol, t2_vol, pred, gt):
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(3.2*n_cols, 3.2*n_rows), facecolor="black")
    if n_rows == 1:
    ...
    for row, z in enumerate(slice_idxs):
        ...
        axes[row, 5].axis("off")
        axes[row, 0].text(-0.22, 0.5, f"Slice {z}", transform=axes[row, 0].transAxes,
                          rotation=90, va="center", ha="center", fontsize=14,
                          color="white", fontweight="bold")
    for col, label in enumerate(col_labels):
        axes[-1, col].text(0.5, -0.14, label, transform=axes[-1, col].transAxes,
                           ha="center", va="top", fontsize=16, color="white", fontweight="bold")
    plt.subplots_adjust(wspace=0.01, hspace=0.01, left=0.08, right=0.99, top=0.99, bottom=0.10)
...
def save_full_comparison(path, pred, gt):
    ...
    def make_rgb(seg):
        ...
        rgb[seg[1, z] > 0.5] = [1.0, 1.0, 0.0]
        rgb[seg[2, z] > 0.5] = [1.0, 0.0, 0.0]
        return rgb
...
def main(args):
    ...
    with torch.no_grad():
        for img, mask, meta in tqdm(loader):
            ...
            # ---- FIXED: NIfTI sebagai label map (bukan 4D) ----

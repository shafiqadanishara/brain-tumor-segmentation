...
# --------------------------------------------------
# OVERLAY BUILDER — single source of truth for colors
# WT=green, TC=yellow, ET=red (nested)
# --------------------------------------------------
...
# --------------------------------------------------
# SUMMARY — multi-slice axial, all sequences
# --------------------------------------------------
...
# --------------------------------------------------
# FULL COMPARISON — uses build_overlay (same colors as summary)
# --------------------------------------------------

def save_full_comparison(path, pred, gt, flair_vol=None):
    """
    pred, gt   : (3, D, H, W)
    flair_vol  : (D, H, W) optional — used as MRI background
    Color: WT=green, TC=yellow, ET=red (same as summary)
    """
    z = get_best_slice(pred)
    mri_bg = flair_vol[z] if flair_vol is not None else np.zeros((pred.shape[2], pred.shape[3]))

    pred_rgb = build_overlay(mri_bg, pred[:, z])
    gt_rgb   = build_overlay(mri_bg, gt[:, z])

    fig, axes = plt.subplots(1, 2, figsize=(10, 5), facecolor="black")
    axes[0].imshow(pred_rgb)
    axes[0].set_title("Prediction", color="white")
    axes[0].axis("off")
    axes[1].imshow(gt_rgb)
    axes[1].set_title("Ground Truth", color="white")
    axes[1].axis("off")

    plt.suptitle("Full Segmentation Comparison", color="white")
    plt.tight_layout()
...
# --------------------------------------------------
# COMPARISON per region — mask only, NO brain
# --------------------------------------------------
...
def main(args):
    ...
    out_root  = Path(f"experiments/ensemble/output_test/{fold_name}")
    ...
    with torch.no_grad():
        ...
        for img, mask, meta in tqdm(loader):
            ...
            case_path = os.path.join(
                "data/raw/ASNR-MICCAI-BraTS2023-GLI-Challenge-TrainingData", case
            )
            ...
            def load_mod(keyword):
                ...
                return np.transpose(
                    nib.load(os.path.join(case_path, f)).get_fdata(), (2, 0, 1)
                )
            ...
            # 128³ visuals
            save_summary(case_dir / "summary_128.png",
                         flair_128, t1_128, t1ce_128, t2_128, pred_np, gt_np)
            save_comparison(case_dir / "comparison_pred_gt_128.png", pred_np, gt_np)
            save_full_comparison(case_dir / "comparison_full_128.png",
                                 pred_np, gt_np, flair_vol=flair_128)

            # Original space visuals
            save_summary(case_dir / "summary_original.png",
                         orig_flair, orig_t1, orig_t1ce, orig_t2, restored, gt_restored)
            save_comparison(case_dir / "comparison_pred_gt_original.png", restored, gt_restored)
            save_full_comparison(case_dir / "comparison_full_original.png",
                                 restored, gt_restored, flair_vol=orig_flair)
            ...
            # NIfTI
            save_nifti(restored.transpose(1, 2, 3, 0), affine,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_json", default="data/folds/test.json")

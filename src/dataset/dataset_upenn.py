...
# src/dataset/dataset_upenn.py
# UPenn-GBM cross-dataset testing
# CLEAN CASES ONLY (BraTS overlap removed)
...
class UPennDataset3D(Dataset):
    ...
    def __init__(
        self,
        root_dir,
        clean_cases_file="clean_upenn_cases.txt"
    ):
        self.root_dir   = root_dir
        self.struct_dir = os.path.join(root_dir, "images_structural")
        self.segm_dir   = os.path.join(root_dir, "images_segm")
        ...
        with open(clean_cases_file, "r") as f:
            clean_cases = {
                line.strip()
                for line in f
                if line.strip()
        ...
        self.cases = sorted([
            d for d in os.listdir(self.struct_dir)
            if (
                os.path.isdir(os.path.join(self.struct_dir, d))
                and d in clean_cases
                and os.path.exists(
                    os.path.join(
                        self.segm_dir,
                        f"{d}_segm.nii.gz"
        ...
        if not self.cases:
            raise RuntimeError(
                f"No valid clean cases found.\n"
                f"Check: {clean_cases_file}"
            )

        print(
            f"[UPennDataset3D] "
            f"{len(self.cases)} clean cases "
            f"(BraTS overlap removed)"
        )
    ...
    def __getitem__(self, idx):
        ...
        meta = {
            ...
            "original_shape":
                original_shape.astype(np.int32),

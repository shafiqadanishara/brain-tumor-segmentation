...
def crop_roi_t1(t1, t1ce, t2, flair, seg):
    ...
    NOTE: only H and W are cropped here. D is intentionally left
    untouched (matches what resize_3d later squashes to 128).

    Returns:
        t1, t1ce, t2, flair, seg, (h1, h2, w1, w2)

        (h1, h2, w1, w2) are the EXACT H/W crop boundaries used here.
        Callers (dataset3D.py) must build the saved `bbox` metadata
        from this return value -- not recompute it independently --
        otherwise restore_to_original() in postprocess.py will place
        predictions in the wrong region at test time.
    """
    mid = t1.shape[0] // 2
    slice_img = t1[mid, :, :]
    ...
    return t1, t1ce, t2, flair, seg, (h1, h2, w1, w2)

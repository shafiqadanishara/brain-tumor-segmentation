...
class BraTSDataset3D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # -----------------
        # preprocess
        # bbox MUST come from crop_roi_t1's actual crop boundaries,
        # not a separately-computed full-volume argwhere -- otherwise
        # restore_to_original() places predictions in the wrong region.
        # D is never cropped by crop_roi_t1, so its bbox range is the
        # full original depth.
        # -----------------
        original_depth = t1.shape[0]
        ...
        h0, h1, w0, w1 = crop_hw
        bbox = np.array([0, original_depth, h0, h1, w0, w1])

...
def plot_all(history_path, out_dir=None):
    ...
    Plots saved:
        {modality}_loss.png
        {modality}_dsc_mean.png
        {modality}_dsc_regions.png
        {modality}_hd95_mean.png
        {modality}_hd95_regions.png
        {modality}_prec.png
        {modality}_sens.png
        {modality}_spec.png
        {modality}_acc.png
    ...
    _plot_curve(
        epochs,
        history["train_hd95_mean"],
        history["val_hd95_mean"],
        title=f"HD95 Mean — {modality}",
        ylabel="HD95 (mm)",
        save_path=os.path.join(out_dir, f"{modality}_hd95_mean.png"),
    )

    # ---- HD95 per region ----
    plt.figure(figsize=(8, 5))
    for region, color in colors.items():
        plt.plot(epochs, history[f"train_hd95_{region}"],
                 label=f"Train {region}", color=color, linestyle="--")
        plt.plot(epochs, history[f"val_hd95_{region}"],
                 label=f"Val {region}",   color=color, linestyle="-")
    plt.title(f"HD95 per Region — {modality}")
    plt.xlabel("Epoch")
    plt.ylabel("HD95 (mm)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, f"{modality}_hd95_regions.png"), dpi=150)
    plt.close()
    ...
    print(f"  Files: {modality}_loss.png, {modality}_dsc_mean.png, "
          f"{modality}_dsc_regions.png, {modality}_hd95_mean.png, "
          f"{modality}_hd95_regions.png, {modality}_prec.png, "
